...
#'
#'@description  This function will sample a set of alternate partners from a data.frame for a set of actual unions in order to
...
#'@param geo a character string giving the name of the variable that identifies the clusters that alternate partners should be
...
#'@param women a data.frame object identifying all the potential "female" partners. Its format should be the same as \code{men}, except
#'         variables should end in "w" rather than "h".
#'@param actual a data.frame identifying identifying the actual couples. This dataset should have the variable identified by \code{geo}
...
#'@param weight A character string that identifies a weight variable for sampling of potential partners. If left \code{NULL}, partners will
#' be sampled with equal probability. This weight variable must exist in both the \code{men} and \code{women} data. 
#'@param keep a vector of character strings identifying additional variables in \code{actual} that should be kept in the results.
#'@param verbose if true, the function will report its progress.
#' @details This program uses the \pkg{wrswoR} package to do fast weighted sampling without replacement. Large sets of alternate spouses and
...
#' @return The output of this program is a data.frame of actual and counterfactual unions. It will keep all variables in the three 
